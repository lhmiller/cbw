...
while true do
        ...
        if PingRunLoop then -- allow one run through loop
            ...
            if ping("192.168.60.1", 5) == 1 then
                ...
                if RouterPingFailCount > 2 then -- must be back on-line
                    emailFunc("usr.minister",
                              "Router Ping Failure.  Willow back on line.", "")
                    RouterRelayCount = 0
            ...
            ------time for action
            if RouterPingFailCount >= PingRetries and RouterPingFailCount %
                PingRetries == 0 then -- nth failure
                pulse("io.relay4", 5)
                print("pulsing relay")
            ...
            if ping("192.168.60.1", 5) == 1 then -- don't even bother to check modem unless router is on-line
                if ping("8.8.8.8", 5) == 1 then
                    if ModemPingFailCount > PingRetries then -- must have been off-line
                        emailFunc("usr.minister",
                                  "Modem Ping Failure. Willow back on line.", "")
                        ModemRelayCount = 0
                ...
                ------time for action
                if ModemPingFailCount >= PingRetries and ModemPingFailCount %
                    PingRetries == 0 and ModemRelayCount < RelayRetries then -- nth failure
                    pulse("io.relay5", 5)
                    pulse("io.relay4", 120)
                    i = file.open(PingFileName, "a")
            ...
            if ping("192.168.60.1", 5) == 1 then -- don't even bother to check Mate unless router is on-line  
                ...
                ------time for action
                if FXPingFailCount >= PingRetries and FXPingFailCount %
                    PingRetries == 0 and FXRelayCount < RelayRetries then -- nth failure
                    pulse("io.relay2", 5)
                    i = file.open(PingFileName, "a")
    ...
    if Hour < 1 and Minute == 0 then
        if NewPingFile then -- allows only one pass through this if statement
            PingFileName = "/usb/" .. Year .. "-" .. Month .. "-" .. Date ..
                               "PingLog.txt"
            NewPingFileFunc()

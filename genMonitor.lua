...
function newGenFileFunc()
    ...
    h:write(
        "Time,GenStartCall,GenRunPrev,GenRun,BatteryVolts,GenChargeStartVolts,Relay1\n")
    h:close()
...
while true do
    ...
    if Second == 15 then -- every 1 minute routine
        ...
        if SecondLoop then -- allows only one pass through loop each cycle.
            ...
            print(
                "GenMon 1 minute Logging at: " .. string.format("%02d", Hour) ..
            ...
            if GenRunPrev == false and GenRun == true then -- Gen started since last cycle
                ...
                emailFunc("usr.minister", "Gen Start.",
                          "Battery volts before start=" ..
                              string.format("%.2f", PrevBatteryVolts))
                GenRunMinutes = 0
            end -- GenRunPrev == false and GenRun == true

            if GenRunPrev == true and GenRun == false then -- Gen just stopped since last cycle
                h:write("Gen stop\n")
                emailFunc("usr.minister", "Gen Stop.",
                          "Battery volts at stop=" ..
                              string.format("%.2f", PrevBatteryVolts) ..
                              ". Generator ran " .. GenRunMinutes .. "minutes.")
            ...
            if GenStartCall ~= GenRun then -- Gen not repsonding to call signal.
                ...
                if MismatchCounter > 3 and MismatchCounter % 10 == 0 and
                    MismatchCounter < 8 then -- must really be a mismatch, alert every 10 minutes for 4 times
                    emailFunc("usr.minister", "Gen Mismatch Failure.",
                              "Call for Start does not match gen-run status.")
            ...
            if GenRunMinutes == 2 and GenChargeStartVolts > reg.register1 - 2 then -- BattV did not increase 2 volts in 2 minutes.  Charge function not working
                h:write("Gen charge failure. " ..
                            string.format("%.2f", reg.register1) .. "\n") -- CBW did not like the reg.register1
                emailFunc("usr.minister", "Charge State Failure.",
                          "Global Charge setting may not be correct.")
